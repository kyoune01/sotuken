...
class opencv():
	...
	def caputure(self):
		...
		if self.Aggregate:
			...
			# リストに個数を代入
			self.AreaCalum.append(self.const)
			# カウントを進める
			self.countloop += 1
			"""
				...
				cv2.imwrite('diff'+str(self.save_con)+'.jpg',self.img_diff_mask)

				f = open('result.csv',"a")
				self.csvWriter = csv.writer(f)
				self.csvWriter.writerow(self.AreaCalum)
				f.close()
			"""
			if self.countloop%50 == 0:
				self.average = sum(self.AreaCalum)/len(self.AreaCalum)
				# 結果を出力
				f.write(str(self.average)+"\n")
				print(self.average)
				self.AreaCalum = []
				cv2.imwrite('now'+str(self.countloop/100)+'.jpg',self.frame)

			if self.countloop%10000 == 0:
				# ファイルを閉じる
				f.close()
				print("Stop")
	...
	""" 「frame」上でクリックを検出したときの処理 """
	def mouse_event(self, event, x, y, flags, param):
		# クリックを検出
		self.clickcheck = True
		# イベントが左クリックのときの処理
		if event == cv2.EVENT_LBUTTONUP:
			# クリックされた座標からHSV情報を取得する
			self.H_point = self.frame_hsv.item(y, x, 0)
			self.Aggregate = []
			# 取得したデータを保存する処理を行う
			self.setValue()
	...
	def setValue(self):
		...
		# mouseevntがすでに起きていた場合の処理
		if self.clickcheck:
			# スライダーの数値分減算する
			self.H_low = self.H_point - 10
			# スライダーの数値分加算する
			self.H_up = self.H_point + 10
		else:
			self.H_up = 70
			self.H_low = 90
	...
	def save(self):
		...
		f = open('result.csv',"a")
		self.csvWriter = csv.writer(f)
		self.csvWriter.writerow(self.result)
		f.close()
...
if __name__ == '__main__':
	...
	cv2.createTrackbar('th','setting',30,100,changeSlider)
	cv2.createTrackbar('blur','setting',0,10,changeSlider)
	cv2.createTrackbar('H','setting',1,12, changeSlider)
	cv2.createTrackbar('S_1','setting',0,250,changeSlider)
	cv2.createTrackbar('V_1','setting',0,250,changeSlider)
	cv2.createTrackbar('S_2','setting',250,250,changeSlider)
	cv2.createTrackbar('V_2','setting',250,250,changeSlider)
	cv2.createTrackbar('Area','setting',10,500,changeSlider)
	cv2.createTrackbar('switch','setting',0,1,switchVal)
	cv2.createTrackbar('save','setting',0,1,save)
	...
	while (True):
		...
		# 「frame」ウィンドウでのマウスイベントを感知
		cv2.setMouseCallback("now", opencv.mouse_event)
