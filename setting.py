...
class opencv():
	...
	def mouse_event(self, event, x, y, flags, param):
		...
		if event == cv2.EVENT_LBUTTONUP:
			...
			self.S_point = self.frame_hsv.item(y, x, 1)
			self.V_point = self.frame_hsv.item(y, x, 2)
			# 取得したデータを保存する処理を行う
			self.setValue()
	...
	def setValue(self):
		...
		if self.clickcheck:
			...
			self.S_slider = cv2.getTrackbarPos('S','setting')
			self.V_slider = cv2.getTrackbarPos('V','setting')
			self.area_slider = cv2.getTrackbarPos('Area','setting')

			# スライダーの数値分減算する
			self.H_low = self.H_point - self.H_slider
			self.V_low = self.V_point - self.V_slider
			self.S_low = self.S_point - self.S_slider
			...
			# スライダーの数値分加算する
			self.H_up = self.H_point + self.H_slider
			self.V_up = self.V_point + self.V_slider
			self.S_up = self.S_point + self.S_slider
	...
	""" switchスライダーが切り替えられたときの関数 """
	def switch(self):
		if self.save_con == 0:
			self.setValue()
			# ファイルを書き込み用として開く
			f =  open("setValue.json","a")
			# 辞書型の変数をjsonとして書き出す
...
if __name__ == '__main__':
	...
	cv2.createTrackbar('H','setting',10,50, changeSlider)
	cv2.createTrackbar('S','setting',100,200,changeSlider)
	cv2.createTrackbar('V','setting',100,200,changeSlider)
	cv2.createTrackbar('Area','setting',10,5000,changeSlider)
